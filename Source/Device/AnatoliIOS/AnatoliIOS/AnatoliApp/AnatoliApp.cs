...
namespace AnatoliIOS
{
	public class AnatoliApp
	{
			...
			if ((DateTime.Now - updateTime) > TimeSpan.FromDays (3)) {
				if (AnatoliClient.GetInstance ().WebClient.IsOnline ()) {
					...
					try {
						...
						alert.AddAction (UIAlertAction.Create ("دوباره تلاش کن", UIAlertActionStyle.Default, async delegate {
							await SyncDataBase ();
						}));
		...
		{
			var b1 = CreateToolBarButton (UIImage.FromFile ("home-256"), () => {
				PushViewController (new FirstPageViewController ());
			});
			if (GetVisibleViewController ().GetType () != typeof(FirstPageViewController))
				b1.TintColor = UIColor.DarkGray;
			var b2 = CreateToolBarButton (UIImage.FromFile ("ic_list_orange_24dp"), () => {
			...
			var b4 = CreateToolBarButton (UIImage.FromFile ("ic_mylist_orange_24dp"), () => {
				PushViewController (new FavoritsViewController ());
			});
			if (GetVisibleViewController ().GetType () != typeof(FavoritsViewController))
				b4.TintColor = UIColor.DarkGray;
			var b5 = CreateToolBarButton (UIImage.FromFile ("126-More-128"), () => {
...
//			return b;
			...
			if (enabled)
				button.SetBackgroundImage(UIImage.FromBundle("ic_shoppingcard_blue_24dp").Scale(new CoreGraphics.CGSize(26, 26)), UIControlState.Normal);
			else
				button.SetBackgroundImage(UIImage.FromBundle("ic_shoppingcard_gray_24dp").Scale(new CoreGraphics.CGSize(26, 26)), UIControlState.Normal);
			...
			_counterLabel = new UILabel(new CGRect(15, 0, 12, 12));
	...
	public static class Extensions
	{
		...
		public static void SetStyle (this UITextField textField)
		{
			...
			border.Frame = new CGRect (0, textField.Frame.Size.Height - width - 1, textField.Frame.Size.Width, 1);
