...
namespace AnatoliIOS.TableViewCells
{
    public partial class OrderHistoryItemSummaryViewCell : BaseTableViewCell
    {
        ...
        public OrderHistoryItemSummaryViewCell(IntPtr handle)
            : base(handle)
        {
        }
        ...
        public void BindCell(OrderItemModel item)
        {
            ...
            {
                var result = await ShoppingCardManager.AddProductAsync(item.product_id, AnatoliApp.GetInstance().DefaultStore.UniqueId, item.item_count);
                if (result)
                {
