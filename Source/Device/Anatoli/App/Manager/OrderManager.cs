    ...
    public class OrderManager : BaseManager<OrderModel>
    {
        public static OrderModel GetOrderByIdAsync(string orderId)
        {
            try
            {
                ...
                return AnatoliClient.GetInstance().DbClient.GetItem<OrderModel>(query);
            }
            catch (Exception)
            {
                return null;
        ...
        public static  OrderModel GetOrderByUniqueId(string uniqueId)
        {
            try
            {
        ...
        public static async Task SyncOrderItemsAsync(Guid customerId, OrderModel order)
        {
            try
            {
                SelectQuery q = new SelectQuery("order_items", new EqFilterParam("order_id", order.order_id.ToString()));
                var l = AnatoliClient.GetInstance().DbClient.GetList<OrderItemModel>(q);
                if (l.Count > 0)
                {
                    return;
                }
        ...
        public static async Task SyncOrdersAsync(Guid customerId)
        {
            try
            {
                ...
                if (orders != null)
                {
                    ...
                    var localOrders = AnatoliClient.GetInstance().DbClient.GetList<OrderModel>(query);
        ...
        public override int UpdateItem(OrderModel model)
        {
